...
def isvalid_word(word, hand, word_list):
    ...
    for key_val in word_freq:
        if key_val not in hand:
            return False
        if word_freq[key_val] > hand[key_val]:
            return False
    return True
#
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(hand) > 0:
        ...
        # Otherwise (the input is not a single period):
            ...
            if not isvalid_word(user_input, hand, word_list):
...
def play_game(word_list):
    ...
    user_hand = None
    while True:
        user_option = input("input n(new game) or r(last hand) or e(exit)")
        if user_option == 'n':
            user_hand = deal_hand(HAND_SIZE)
            play_hand(user_hand, word_list, HAND_SIZE)
        elif user_option == 'r':
            play_hand(user_hand, word_list, HAND_SIZE)
        elif user_option == 'e':
            return
